...
function register(req, res) {
  ...
  try {
    db.transaction(() => {
      db.prepare(`
        INSERT INTO accounts (id, business_name, plan, trial_ends_at, created_at)
        VALUES (?, ?, 'trial', ?, ?)
      `).run(accountId, business_name.trim(), trialEnds, now);

      db.prepare(`
        INSERT INTO properties (id, account_id, name, city, created_at)
        VALUES (?, ?, ?, ?, ?)
      `).run(propertyId, accountId, (pg_name || business_name).trim(), (city || '').trim() || null, now);
...
function resetPassword(req, res) {
  ...
  const record = db.prepare(
    'SELECT * FROM otp_store WHERE mobile = ? AND expires_at > datetime(\'now\')'
  ).get(mobileTrim);
...
function changePassword(req, res) {
  ...
  db.prepare("UPDATE users SET password_hash = ?, updated_at = datetime('now') WHERE
